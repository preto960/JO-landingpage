...
import { Loader2, Search, Shield, Users, UserCheck, UserX, Trash2, ChevronDown, X, Copy, Check } from 'lucide-react'
...
export default function UsersContent({ user }: { user: { name: string; email: string; role: string } }) {
      ...
      {newInviteCode && (
        <div className="p-4 sm:p-6" style={{ background: 'rgba(201,168,76,.05)', border: '1px solid rgba(201,168,76,.15)' }}>
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <p className="text-sm" style={{ fontFamily: "'Jost', sans-serif", color: '#C9A84C', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: '.25rem' }}>
                Nuevo código de invitación
              </p>
              <p className="text-lg sm:text-xl" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#F5F0E8', letterSpacing: '.15em' }}>
                {newInviteCode}
              </p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => handleCopyCode(newInviteCode)}
                className="p-2"
                style={{
                  background: copiedCode === newInviteCode ? 'rgba(34,197,94,.1)' : 'rgba(201,168,76,.1)',
                  border: '1px solid ' + (copiedCode === newInviteCode ? 'rgba(34,197,94,.3)' : 'rgba(201,168,76,.2)'),
                  color: copiedCode === newInviteCode ? '#22c55e' : '#C9A84C',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                }}
              ...
              <button onClick={() => setNewInviteCode(null)} className="p-2" style={{ background: 'none', border: '1px solid rgba(245,240,232,.1)', color: 'rgba(245,240,232,.4)', cursor: 'pointer' }}>
      ...
      <div className="p-4 sm:p-6" style={cardStyle}>
        <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(201,168,76,.15)', background: 'rgba(201,168,76,.05)' }}>
          ...
          <button
            ...
            style={{
              ...
              textTransform: 'uppercase', height: '2.5rem', padding: '0 1.25rem',
              background: showInviteForm ? 'transparent' : '#C9A84C', color: showInviteForm ? 'rgba(245,240,232,.5)' : '#0A0A0A',
              border: showInviteForm ? '1px solid rgba(245,240,232,.12)' : 'none',
              cursor: 'pointer', transition: 'all .3s', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '.5rem',
            }}
          >
        ...
        {showInviteForm && (
          <div className="p-4 mb-4" style={{ background: 'rgba(10,10,10,.4)', border: '1px solid rgba(245,240,232,.06)' }}>
            ...
            <button
              ...
              style={{
                fontFamily: "'Jost', sans-serif", fontSize: '.78rem', fontWeight: 500, letterSpacing: '.18em',
                textTransform: 'uppercase', height: '2.5rem', padding: '0 1.5rem',
                background: '#C9A84C', color: '#0A0A0A', border: 'none', cursor: 'pointer',
                transition: 'background .3s', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '.5rem',
              }}
            >
              Generar Código
            </button>
        ...
        {loadingInvites ? (
          ...
          <p className="text-center py-6" style={{ fontFamily: "'Jost', sans-serif", fontSize: '.85rem', color: 'rgba(245,240,232,.3)' }}>
            No hay códigos de invitación. Crea uno para invitar nuevos usuarios.
          </p>
        ) : (
              ...
              return (
                <div key={invite.id} className="flex items-center justify-between gap-3 p-3 flex-wrap" style={{ background: isDisabled ? 'rgba(245,240,232,.01)' : 'rgba(245,240,232,.02)', border: '1px solid rgba(245,240,232,.04)', opacity: isDisabled ? 0.4 : 1 }}>
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <span className="hidden sm:inline-block px-2 py-0.5" style={{ fontFamily: "'Jost', sans-serif", fontSize: '.6rem', textTransform: 'uppercase', letterSpacing: '.08em', color: ROLE_COLORS[invite.role as Role], background: 'rgba(201,168,76,.05)', border: '1px solid rgba(201,168,76,.1)' }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isSuperAdmin && (
                      <button onClick={() => handleDeleteInvite(invite.id)} className="p-1.5" style={{ background: 'none', border: 'none', color: 'rgba(248,113,113,.4)', cursor: 'pointer' }}>
      ...
      <div className="p-4 sm:p-6" style={cardStyle}>
        ...
        <div className="flex gap-3 mb-4 flex-col sm:flex-row">
          ...
          <select
            ...
            style={{ ...inputBase, width: 'auto', minWidth: '10rem', appearance: 'none', cursor: 'pointer' }}
          >
        ...
        {loading ? (
          ...
          <p className="text-center py-12" style={{ fontFamily: "'Jost', sans-serif", fontSize: '.85rem', color: 'rgba(245,240,232,.3)' }}>
          ...
          <div className="flex flex-col">
            ...
            <div className="hidden sm:grid sm:grid-cols-12 gap-3 pb-3 mb-2" style={{ borderBottom: '1px solid rgba(245,240,232,.06)' }}>
              {['Usuario', 'Rol', 'Estado', 'Último acceso', 'Acciones'].map((h) => (
                <span key={h} style={{ fontFamily: "'Jost', sans-serif", fontSize: '.65rem', textTransform: 'uppercase', letterSpacing: '.12em', color: 'rgba(245,240,232,.25)' }} className={h === 'Usuario' ? 'col-span-4' : h === 'Acciones' ? 'col-span-3 text-right' : 'col-span-2'}>
                  {h}
                </span>
              ))}
            </div>
                ...
                <div key={u.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-3 items-center py-3" style={{ borderBottom: '1px solid rgba(245,240,232,.04)' }}>
                  ...
                  <div className="sm:col-span-4 flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 flex-shrink-0 flex items-center justify-center" style={{ background: 'rgba(201,168,76,.08)', border: '1px solid rgba(201,168,76,.12)' }}>
                      <span className="text-xs" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#C9A84C', fontWeight: 300 }}>
                    ...
                    {!isSelf && isSuperAdmin ? (
                      <div className="relative">
                        <select
                          ...
                          className="w-full"
                          style={{
                            fontFamily: "'Jost', sans-serif", fontSize: '.7rem', fontWeight: 400, letterSpacing: '.08em',
                            textTransform: 'uppercase', padding: '.375rem .5rem',
                            background: 'rgba(10,10,10,.6)', border: '1px solid rgba(245,240,232,.07)',
                            borderRadius: 0, color: ROLE_COLORS[u.role as Role], appearance: 'none',
                      ...
                      <span className="inline-block px-2 py-0.5" style={{ fontFamily: "'Jost', sans-serif", fontSize: '.6rem', textTransform: 'uppercase', letterSpacing: '.08em', color: ROLE_COLORS[u.role as Role], background: 'rgba(201,168,76,.05)', border: '1px solid rgba(201,168,76,.1)' }}>
                    ...
                    {!isSelf && (
                      <>
                        <button
                          ...
                          disabled={isBusy}
                          className="p-1.5"
                          style={{ background: 'none', border: 'none', color: u.isActive ? 'rgba(248,113,113,.4)' : 'rgba(34,197,94,.5)', cursor: isBusy ? 'not-allowed' : 'pointer', opacity: isBusy ? 0.5 : 1 }}
                        ...
                        {isSuperAdmin && (
                          <button
                            ...
                            disabled={isBusy}
                            className="p-1.5"
                            style={{ background: 'none', border: 'none', color: 'rgba(248,113,113,.3)', cursor: isBusy ? 'not-allowed' : 'pointer', opacity: isBusy ? 0.3 : 1 }}
          ...
          <div className="flex items-center justify-between gap-4 mt-4 pt-4" style={{ borderTop: '1px solid rgba(245,240,232,.06)' }}>
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={page === 1}
                style={{ fontFamily: "'Jost', sans-serif", fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', padding: '.5rem 1rem', background: 'transparent', color: page === 1 ? 'rgba(245,240,232,.15)' : 'rgba(245,240,232,.4)', border: '1px solid rgba(245,240,232,.08)', cursor: page === 1 ? 'not-allowed' : 'pointer' }}
              >
                Anterior
              </button>
              <button
                ...
                style={{ fontFamily: "'Jost', sans-serif", fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', padding: '.5rem 1rem', background: 'transparent', color: page === totalPages ? 'rgba(245,240,232,.15)' : 'rgba(245,240,232,.4)', border: '1px solid rgba(245,240,232,.08)', cursor: page === totalPages ? 'not-allowed' : 'pointer' }}
              >
                Siguiente
              </button>
            </div>
